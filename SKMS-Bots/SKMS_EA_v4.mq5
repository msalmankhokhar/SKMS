...
input bool       UseSMAConfirmation    = false; // SMA Confirmation (ON/OFF)
...
//+------------------------------------------------------------------+
//|  GET SMA BIAS                                                   |
//|  Returns: 1 = Bullish (SMA200 > SMA700, buy only)              |
//|           -1 = Bearish (SMA200 < SMA700, sell only)            |
//|            0 = No filter (SMA confirmation OFF)                |
//+------------------------------------------------------------------+
int GetSMABias()
{
   if(!UseSMAConfirmation) return 0;

   double fastBuf[1], slowBuf[1];
   if(CopyBuffer(hSMA_Fast, 0, 1, 1, fastBuf) <= 0) return 0;
   if(CopyBuffer(hSMA_Slow, 0, 1, 1, slowBuf) <= 0) return 0;

   double smaFast = fastBuf[0]; // SMA 200
   double smaSlow = slowBuf[0]; // SMA 700

   if(smaFast > smaSlow) return  1; // SMA200 > SMA700 → buy only
   if(smaFast < smaSlow) return -1; // SMA200 < SMA700 → sell only
   return 0;
}
      ...
      if(swingBias == BULLISH)
      {
         ...
         if(confirmLow.price > 0 && C < confirmLow.price && confirmLow.confirmed)
         {
            ...
            if(!bearishChochActive && !bullishChochActive && !IsTradeOpen())
            {
               ...
               // SMA confirmation: only buy when SMA200 > SMA700
               int smaBias = GetSMABias();
               if(volumeOk && (smaBias >= 0))
               {
      ...
      else if(swingBias == BEARISH)
      {
         ...
         if(confirmHigh.price > 0 && C > confirmHigh.price && confirmHigh.confirmed)
         {
            ...
            if(!bullishChochActive && !bearishChochActive && !IsTradeOpen())
            {
               ...
               // SMA confirmation: only sell when SMA200 < SMA700
               int smaBias2 = GetSMABias();
               if(volumeOk && (smaBias2 <= 0))
               {

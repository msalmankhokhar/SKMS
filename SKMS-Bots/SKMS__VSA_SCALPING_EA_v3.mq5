...
input bool     OnlyAPlusSetup     = false;      // Only A+ Setups (ON/OFF)
...
bool   cabLowSwept;             // CAB low swept/broken before entry (buy A+)
bool   cabHighSwept;            // CAB high swept/broken before entry (sell A+)
...
void ProcessSetupTracking(double O, double C, double H, double L, long V)
{
   ...
   if(swingBias == BULLISH && canBuy)
   {
      ...
      {
         // New higher volume bearish CAB appeared
         if(C < O && V > cabVolume)
         {
            // If trade is open → close it first, then activate new CAB
            if(IsTradeOpen())
            {
               for(int i = PositionsTotal()-1; i >= 0; i--)
               {
                  ...
                  if(trade.PositionClose(ticket))
                     Print("Trade closed: new higher vol CAB found");
                  else
            ...
            Print("CAB updated (higher vol) | New CAB High=", H, " Low=", L,
                  " Vol=", V, " | Waiting for entry confirmation");
         }

         // Below logic only when no trade is open
         if(!IsTradeOpen())
         {
         // Track if CAB low swept or broken (for A+ setup)
         if(L < absoluteSlPoint)
         {
            absoluteSlPoint = L;
            cabLowSwept     = true;
            Print("AbsoluteSL updated to ", L, " | CAB low swept/broken");
         }
         ...
         {
            // Validity check: entry candle volume must be < CAB volume
            if(V >= cabVolume)
            {
               Print("Entry INVALID: entry vol(", V, ") >= CAB vol(", cabVolume, ") | Waiting...");
               trackedEntry = H; // keep tracking, wait for valid candle
            }
            else
            {
               // A+ check
               bool aPlusOk = !OnlyAPlusSetup || cabLowSwept;
               if(!aPlusOk)
               {
                  Print("Not A+ setup: CAB low not swept before entry | Skipping");
                  trackedEntry = H;
               }
               else
               {
                  ...
                  Print("BUY Entry", (OnlyAPlusSetup ? " [A+]" : ""),
                        " | Ask=", ask, " SL=", sl, " TP=", tp,
                        " BE=", be, " Lot=", lot,
                        " EntryVol=", V, " CABVol=", cabVolume);
   ...
   else if(swingBias == BEARISH && canSell)
   {
      ...
      {
         // New higher volume bullish CAB appeared
         if(C > O && V > cabVolume)
         {
            // If trade is open → close it first, then activate new CAB
            if(IsTradeOpen())
         ...
         if(!IsTradeOpen())
         {
         // Track if CAB high swept or broken (for A+ setup)
         if(H > absoluteSlPoint)
         {
            absoluteSlPoint = H;
            cabHighSwept    = true;
            Print("AbsoluteSL updated to ", H, " | CAB high swept/broken");
         }
         ...
         if(C < trackedEntry && C < O)
         {
            ...
            {
               Print("Entry INVALID: entry vol(", V, ") >= CAB vol(", cabVolume, ") | Waiting...");
               trackedEntry = L; // keep tracking, wait for valid candle
            }
            else
            {
               // A+ check
               bool aPlusOk = !OnlyAPlusSetup || cabHighSwept;
               if(!aPlusOk)
               {
                  Print("Not A+ setup: CAB high not swept before entry | Skipping");
                  trackedEntry = L;
               }
               else
               {
                  ...
                  Print("SELL Entry", (OnlyAPlusSetup ? " [A+]" : ""),
                        " | Bid=", bid, " SL=", sl, " TP=", tp,
                        " BE=", be, " Lot=", lot,
                        " EntryVol=", V, " CABVol=", cabVolume);
